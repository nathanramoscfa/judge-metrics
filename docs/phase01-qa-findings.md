...
| 6.8 | First local `--post`: V5.3 failed with an empty database — the migration round-trip test (V2.3) had downgraded the configured database to base and back and the API tests had purged their fixture ingest before the Playwright smoke ran (finding 2.5 reaching the verify script). | Implementation bug (ordering) | `--post` and `--all` run the smoke test and the web suites before the Python suites, with the reason in the docstring; the operator re-runs `uv run poe ingest-fjc` after `--post`, as after `uv run poe check`. |
