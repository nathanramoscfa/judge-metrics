...
provenance trace, the metrics API, and corrections intake) is next, in
a fresh conversation from
...
1. **Phase 3 Step 3.** `judgemetrics provenance trace
   <observation id>` and `GET /api/v1/metrics/{id}/provenance` walking
   observation → members → source records → raw artifacts; the metrics
   API (`/metrics`, `/judges/{id}/metrics`, `/courts/{id}/metrics`,
   `/metrics/compare`, `/coverage` extended) serving the current
   observations with suppression applied (numerator, denominator,
   value, and interval withheld below the threshold); `POST
   /api/v1/corrections` with the encrypted contact.
