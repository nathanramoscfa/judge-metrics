...
engine, `metrics compute|verify`, and the golden metric tests) is next,
in a fresh conversation from
...
- The synthetic generator's `truth/metrics.json` computes the windowed
  ...
  arrive under `TRUTH_VERSION` 2 in Phase 3 Step 2, implementing the
  semantics `docs/METHODOLOGY.md` states (the registry defines them
  since Step 1). `age_at_filing` is filled even when a date of birth is
  missing; a revocation is recorded after its case closed; the
...
- The synthetic connector derives `new_case` and `reconviction` justice
  ...
  frame reads outcomes from `justice_events`, so Step 2's snapshot
  loader derives the other-case outcomes from the merged person's cases
  (recorded in its task block); the in-memory frame the property tests
  build uses the truth generator's own outcomes and is exact.
...
1. **Phase 3 Step 2.** The snapshot export (`metrics/snapshot.py`,
   Parquet under a content hash, DuckDB views), the compute dispatch
   over the Step 1 frame, suppression, publishing with members and
   `superseded_at` history, `judgemetrics metrics compute|verify`, the
   `compute-metrics` target, pipeline step 13, `source.coverage_*` and
   `observable_outcomes` filled by the connectors, `TRUTH_VERSION` 2
   with the golden fixture regenerated, and
