...
2026-09-18, and Step 4 (the case API, case pages, and coverage v0) later
the same day; Step 5 (property tests, the golden suite, and the scratch
test database, branch `feature/phase02-step5-property-golden`) begins in
...
- The API integration modules that need case data (`test_api_cases`,
  `test_api_coverage`, `test_api_judges`, `test_api_search`,
  `test_query_counts`) commit a golden ingest per module through the
  `golden_fixture` fixture, which purges the whole `synthetic` source
  before and after because the golden and demo datasets share natural
  keys (judge codes, participant ids, case numbers). A local test run
  therefore removes the demo seed as well as the FJC live ingest (the
  migration round-trip test); run `uv run poe ingest-fjc` and `uv run
  poe seed` afterwards. Each golden ingest's two merges leave
  append-only `audit_log` rows behind (ids and counts only); Step 5's
  scratch test database ends both effects.
