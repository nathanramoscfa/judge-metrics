...
**Phase 3 — First Milestone: Metrics Registry, Provenance, and the
End-to-End Bootstrap.** Not started. Phase 2 (Synthetic Justice
Dataset, Entity Resolution, and Case Timelines) completed on 2026-09-18
with Step 6 (QA and `scripts/verify_phase02.py`) and is tagged
`v0.2.0-phase-2` on the squash-merged commit. Phase 1 (Foundation,
Canonical Schema, and the FJC Judge Slice) completed on 2026-09-16 and
is tagged `v0.1.0-phase-1`. Phase 3 begins in a fresh conversation
after the planning kit is re-exported (`uv run poe kit`) and
`docs/phase03-roadmap.md` is authored from [`ROADMAP.md`](../ROADMAP.md)
§4 Phase 3; its carry-over checklist is the last section of
[`docs/phase02-qa-findings.md`](phase02-qa-findings.md).
...
1. **Phase 3 planning.** Re-export the planning kit (`uv run poe kit`)
   and author `docs/phase03-roadmap.md` from `ROADMAP.md` §4 Phase 3,
   folding in the Phase 2 carry-over checklist
   (`docs/phase02-qa-findings.md`).
2. **First milestone (`v0.3.0-phase-3`).** The metrics registry with
   the brief's outcome definitions, provenance tracing
